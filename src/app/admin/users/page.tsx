...
export default function AdminUsersPage() {
    ...
    const fetchUsers = async () => {
        setIsLoading(true);
        try {
            // Aggregate customer profiles directly from orders table (prevents non-existent view console errors)
            const { data: ordersData, error: ordersError } = await supabase
                .from('orders')
                .select('customer_email, total_amount, created_at, user_id');

            if (ordersError) {
                console.warn("Could not fetch orders for customer directory:", ordersError.message);
                setUsers([]);
                setFilteredUsers([]);
                return;
            }

            const statsMap = new Map<string, CustomerProfile>();
            ...
            const userList: CustomerProfile[] = Array.from(statsMap.values());
            // Default sort by total spent desc
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
                <div className="col-span-1 md:col-span-2 bg-gradient-to-br from-[#85adff] to-[#699cff] p-8 rounded-xl shadow-[0_24px_48px_-12px_rgba(133,173,255,0.2)] relative overflow-hidden group">
                    <div className="relative z-10">
                        <p className="font-['Inter'] text-[#000000]/80 text-sm font-medium">Total de Clientes Activos</p>
                ...
                <div className="bg-[#19191c] p-6 rounded-xl border border-[#48474a]/10 relative overflow-hidden group">
                    ...
                    <p className="font-['Inter'] text-[#adaaad] text-sm relative z-10">Gasto Promedio por Cliente</p>
                ...
                <div className="bg-[#19191c] p-6 rounded-xl border border-[#48474a]/10 relative overflow-hidden group">
                    ...
                    <p className="font-['Inter'] text-[#adaaad] text-sm relative z-10">Promedio de Pedidos por Cliente</p>
            ...
            <section className="bg-[#19191c] rounded-xl overflow-hidden shadow-2xl border border-[#48474a]/10">
                <div className="px-8 py-6 flex flex-col xl:flex-row justify-between xl:items-center border-b border-[#48474a]/10 gap-4">
                    <h3 className="font-['Manrope'] text-[#f9f5f8] text-xl font-bold">Directorio de Clientes</h3>
                        ...
                        <thead>
                            <tr className="bg-[#131315]/50 border-b border-[#48474a]/10">
                                <th className="px-8 py-4 font-['Manrope'] text-[11px] font-extrabold uppercase tracking-widest text-[#adaaad] cursor-pointer hover:text-[#f9f5f8] transition-colors" onClick={() => handleSort('email')}>
                                    <div className="flex items-center gap-1">Usuario {sortField === 'email' && <ArrowUpDown className="w-3 h-3" />}</div>
                                </th>
                                ...
                                <th className="px-8 py-4 font-['Manrope'] text-[11px] font-extrabold uppercase tracking-widest text-[#adaaad] cursor-pointer hover:text-[#f9f5f8] transition-colors" onClick={() => handleSort('isAdmin')}>
                                    <div className="flex items-center gap-1">Rol {sortField === 'isAdmin' && <ArrowUpDown className="w-3 h-3" />}</div>
                                </th>
                                <th className="px-8 py-4 font-['Manrope'] text-[11px] font-extrabold uppercase tracking-widest text-[#adaaad] cursor-pointer hover:text-[#f9f5f8] transition-colors" onClick={() => handleSort('totalOrders')}>
                                    <div className="flex items-center gap-1">Pedidos {sortField === 'totalOrders' && <ArrowUpDown className="w-3 h-3" />}</div>
                                </th>
                                <th className="px-8 py-4 font-['Manrope'] text-[11px] font-extrabold uppercase tracking-widest text-[#adaaad] cursor-pointer hover:text-[#f9f5f8] transition-colors" onClick={() => handleSort('totalSpent')}>
                                    <div className="flex items-center gap-1">Total Gastado {sortField === 'totalSpent' && <ArrowUpDown className="w-3 h-3" />}</div>
                                </th>
                            ...
                            {filteredUsers.length === 0 ? (
                                ...
                                filteredUsers.map((user, idx) => (
                                    <tr key={idx} className="group hover:bg-[#1f1f22] transition-colors duration-200">
                                        ...
                                        <td className="px-8 py-6">
                                            {user.id ? (
                                                <span className="bg-[#69f6b8]/10 text-[#69f6b8] px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider line-clamp-1 border border-[#69f6b8]/20">Activo</span>
                                            ) : (
                                                <span className="bg-[#48474a]/20 text-[#adaaad] px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider line-clamp-1 border border-[#48474a]/20" title="Usuario Invitado (Sin Registro)">Invitado</span>
                                            )}
                                        </td>
                                        <td className="px-8 py-6">
                                            {user.isAdmin ? (
                ...
                <div className="px-8 py-6 bg-[#131315]/30 border-t border-[#48474a]/10 flex items-center justify-between">
                    <p className="text-xs text-[#adaaad] font-medium">Mostrando <span className="text-[#f9f5f8]">{filteredUsers.length}</span> de <span className="text-[#f9f5f8]">{users.length}</span> clientes</p>
