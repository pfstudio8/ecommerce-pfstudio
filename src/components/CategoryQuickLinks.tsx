...
import { Shirt, Trophy, Compass, Palette, Sparkles, ArrowUpRight } from "lucide-react";
...
export default function CategoryQuickLinks() {
    const categories = [
        ...
        },
        {
            ...
            icon: Compass,
            color: "from-amber-500/10 via-amber-500/5 to-transparent hover:from-amber-500/20",
            borderColor: "hover:border-amber-500/40"
        },
        {
            name: "Accesorios",
            description: "Detalles & Drops",
            href: "/?cat=Accesorios#productos",
            icon: Sparkles,
            color: "from-purple-500/10 via-purple-500/5 to-transparent hover:from-purple-500/20",
    ...
    return (
        <section id="categorias" className="py-16 bg-[var(--background)] relative z-10 border-b border-white/5">
            <div className="container mx-auto px-6 md:px-8 max-w-[1400px]">
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
                    <div>
                        <span className="text-xs font-black uppercase tracking-[0.3em] text-[var(--color-main)] mb-2 block">
                    ...
                    {categories.map((cat, idx) => {
                        ...
                        return (
                            <Link
                                ...
                                className={`group relative block p-6 rounded-2xl border border-white/10 bg-gradient-to-br ${cat.color} ${cat.borderColor} hover:-translate-y-1.5 backdrop-blur-sm transition-all duration-300 overflow-hidden ${cat.highlight ? 'col-span-2 md:col-span-1 shadow-lg shadow-rose-950/20' : ''}`}
                            >
                                ...
                                <div className="relative z-10 flex flex-col h-full justify-between gap-8">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            {cat.badge && (
                                                <span className={`text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider ${cat.highlight ? 'bg-gradient-to-r from-rose-500 to-red-600 text-white' : 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'}`}>
                                                    {cat.badge}
                                                </span>
                                            )}
                                        </div>
